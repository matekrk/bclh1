...
def cl_args(clgroup, show_det_multi_head=False):
    ...
    Args:
        ...
        show_det_multi_head (bool); Whether option `show_det_multi_head` should
            be shown.
    """
    ...
    clgroup.add_argument('--det_multi_head', action='store_true',
                         help='If a multi-head network is used for EWC and ' +
                              'this option is enabled, then the deterministic' +
                              ' uncertainty from the found MAP will be used ' +
                              '(which only captures aleatoric uncertainty). ' +
                              'Hence, a Dirac rather than a Normal posterior ' +
                              'is used.')
...
def check_invalid_args_ewc(config):
    ...
    if hasattr(config, 'cl_scenario') and not (config.cl_scenario == 1 or \
        ...
        warnings.warn('Setting "val_sample_size" to 1.')
        config.val_sample_size = 1
    if hasattr(config, 'det_multi_head') and config.det_multi_head and \
            config.val_sample_size > 1:
        warnings.warn('Dirac posterior used. Setting "val_sample_size" to 1.')
        config.val_sample_size = 1
