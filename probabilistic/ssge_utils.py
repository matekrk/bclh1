...
def cw(first_sample: torch.Tensor, second_sample: torch.Tensor, gamma: Union[torch.Tensor, float],
       cw_normality_scale_factor = 0.28209479177, cw_scale_factor = 0.56418958354) -> torch.Tensor:
...
### MMD ###
def mmd(x: torch.Tensor, y: torch.Tensor, kernel="rbf"):
...
def pairwise_distances(a: torch.Tensor, b: torch.Tensor, p=2):
    ...
    :param a: A tensor containing one batch of n points of dimension d. i.e. of size [n, d]
    :param b: A tensor containing one batch of n points of dimension d. i.e. of size [n, d]
    ...
    if len(a.shape) != 2:
        raise ValueError("Invalid shape for a. Must be [n, d] but got", a.shape)
    if len(b.shape) != 2:
        raise ValueError("Invalid shape for a. Must be [n, d] but got", b.shape)
    return (a.unsqueeze(2) - b.unsqueeze(1)).abs().pow(p).sum(3)

def sink_stabilized(data_a, data_b, reg, numItermax=1000, tau=1e2, stopThr=1e-9, warmstart=None, print_period=20):

    device = data_a.device

    M = pairwise_distances(data_a, data_b)
    na = M.size()[0]
    nb = M.size()[1]

    a = Variable(torch.ones((na,)) / na).to(device)
    b = Variable(torch.ones((nb,)) / nb).to(device)
