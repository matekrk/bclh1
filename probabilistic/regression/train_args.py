...
def train_args(tgroup, show_prior_variance=True, show_ll_dist_std=True,
               show_local_reparam_trick=False, show_kl_scale=False, show_radial_bnn=False,
               show_gamma=False, show_task_id_fixemb=False, show_task_id_emb=False, 
               show_gamma_cw=True, show_gamma_mmd=True, show_gamma_ot=True):
    """This is a helper function of the function :func:`parse_cmd_arguments` to
    ...
    Args:
        ...
        show_gamma (bool): Whether the option
            `gamma` should be provided.
        show_task_id_fixemb (bool): Whether the option
            should be provided.
        show_task_id_emb (bool): Whether the option
            `show_task_id_emb` should be provided.
        show_gamma_cw (bool): Whether the option
            `gamma_cw` should be provided.
        show_gamma_mmd (bool): Whether the option
            `gamma_mmd` should be provided.
        show_gamma_ot (bool): Whether the option
            `gamma_ot` should be provided.
    """
    ...
    if show_radial_bnn:
        tgroup.add_argument('--radial_bnn', action='store_true',
                        help='Sample the weights of the BNN using a `Radial ' +
                             ...
                             'dimensions.')
    if show_gamma:
        tgroup.add_argument('--gamma', type=float, default=0.,
                        help='A scaling factor for extending the Bayesianility ' +
                             'using sim layer term. Suming off-diagonal elements.' +
                             'Default: %(default)s.')
    if show_task_id_fixemb:
        tgroup.add_argument('--fixed_embedding', type=int, default=-1,
                        help='Which task embedding should be used for testing,\
                                either indicated or turn off (-1)' + 
                             'Default: %(default)s.')
    if show_task_id_emb:
        tgroup.add_argument('--task_id_emb', type=int, default=-1,
                        help='Which task embedding should be used for training,\
                                either corresponding to data (-1) or fixed (indicated)' +
                             'Default: %(default)s.')
    if show_gamma_cw:
        tgroup.add_argument('--gamma_cw', type=float, default=0.,
                        help='A scaling factor for CW measure between the following ' +
                             'posteriors.' +
                             'Default: %(default)s.')
    if show_gamma_mmd:
        tgroup.add_argument('--gamma_mmd', type=float, default=0.,
                        help='A scaling factor for CW measure between the following ' +
                             'posteriors.' +
                             'Default: %(default)s.')
    if show_gamma_ot:
        tgroup.add_argument('--gamma_ot', type=float, default=0.,
                        help='A scaling factor for CW measure between the following ' +
                             'posteriors.' +
                             'Default: %(default)s.')
