...
def compute_acc(task_id, data, mnet, hnet, device, config, shared,
                ...
                alphas=None, which_cond_id=-1, inspect_first=0, return_wnorms=False,
                return_sim_layers=False):
    ...
    correct output units depending on the CL scenario) IF NOT which_cond_id provided.
    ...
    if hnet is None:
        ...
        if hasattr(config, 'supsup_task_inference') and \
            ...
            hnet_out = hnet.forward(cond_id=which_cond_id)
    ...
    for j in range(num_w_samples):
        ...
        while n_processed < num_samples:
            ...
            if inspect_first > 0 and inspect_first < curr_bs and inspect_first > n_processed:
                ...
                Y_sel = mnet.forward(X_sel, weights=W, **kwargs)
    ...
    if inspect_first > 0:
        ...
        pred_all_acc = 100. * np.sum(pred_all_sel == labels_sel) / inspect_first
        ...
        return_vals.inspect_first_acc_all = pred_all_acc
...
def out_units_of_task(config, data, task_id, num_trained):
    ...
    if hasattr(config, 'multi_head'):
        ...
        if config.split_head_cl3:
            if config.fixed_embedding != -1:
                allowed_outputs = list(range(config.fixed_embedding * data.num_classes,
                                             num_trained * data.num_classes))
            else:
                allowed_outputs = list(range(task_id * data.num_classes,
                                            (task_id+1) * data.num_classes))
    ...
    with open(os.path.join(config.out_dir, summary_fn), 'w') as f:
        for k, v in shared.summary.items():
            if isinstance(v, list):
                ...
                if len(v.shape) == 1:
                    f.write('%s %s\n' % (k, misc.list_to_str(list(v))))
                else:
                    assert len(v.shape) == 2
                    for row in range(v.shape[0]):
                        f.write('%s %s\n' % (k, misc.list_to_str(list(v[row]))))
...
def checkpoint_nets_after_base(config, shared, task_id, mnet, hnet, hhnet=None, dis=None):
    """Checkpoint the main and (if existing) the current hypernet. After (at least) the first task.
    ...
    assert config.save_after_base != ""

    dir_to_save = os.path.join(config.save_after_base, f"task_{task_id}/")
    ...
    tckpt.save_checkpoint({'state_dict': mnet.state_dict(),
                           perf_key: perf_val},
        dir_to_save + "main", perf_score, train_iter=None,
        timestamp=ts)
    if hnet is not None:
        tckpt.save_checkpoint({'state_dict': hnet.state_dict(),
                               perf_key: perf_val},
            dir_to_save + "hnet", perf_score, train_iter=None,
            timestamp=ts)
    if hhnet is not None:
        tckpt.save_checkpoint({'state_dict': hhnet.state_dict(),
                               perf_key: perf_val},
            dir_to_save + "hhnet", perf_score, train_iter=None,
            timestamp=ts)
    if dis is not None:
        tckpt.save_checkpoint({'state_dict': dis.state_dict(),
                               perf_key: perf_val},
            dir_to_save + "dis", perf_score, train_iter=None,
            timestamp=ts)

def load_networks_from_base(path, task_id, device, logger, mnet, hnet, hhnet=None,
                  dis=None, strict=False):
    """Load checkpointed networks. Assume prior training on (at least) one task.
    ...
    Returns:
        (float): The performance score of the checkpoint.
    """
    ckpt_dict, score = tckpt.load_checkpoint(os.path.join(path, f"task_{task_id}", "main"),
        mnet, device=device, ret_performance_score=True, strict=strict)
    if hnet is not None:
        tckpt.load_checkpoint(os.path.join(path, f"task_{task_id}", "hnet"), hnet,
                              device=device)
    if hhnet is not None:
        tckpt.load_checkpoint(os.path.join(path, f"task_{task_id}", "hhnet"), hhnet,
                              device=device)
    if dis is not None:
        tckpt.load_checkpoint(os.path.join(path, f"task_{task_id}", "dis"), dis,
                              device=device)
...
def fit_targets_to_softmax(config, shared, device, data, task_id, targets):
    ...
    elif config.cl_scenario == 3 and (config.fixed_embedding != -1 or not config.split_head_cl3 and \
