...
def generate_datasets(config, logger, writer):
    ...
    if config.num_tasks > 1:
        ...
        if writer is not None:
            ...
            writer.add_figure('train/all_tasks/data_dist', plt.gcf(),
        ...
        if writer is not None:
            ...
            writer.add_figure('train/all_tasks/cond_entropy', plt.gcf(),
                              close=not show_plots)
            if show_plots:
                misc.repair_canvas_and_show_fig(plt.gcf())
        ...
        if writer is not None:
            full_data.plot_optimal_classification(title='Class-Boundaries',
                     input_mesh=input_mesh, sketch_components=True, show=False)
            writer.add_figure('train/all_tasks/class_boundaries', plt.gcf(),
                              close=not show_plots)
            if show_plots:
                misc.repair_canvas_and_show_fig(plt.gcf())
        ...
        if writer is not None:
            ...
            writer.add_figure('train/all_tasks/class_boundaries_train', plt.gcf(),
                              close=not show_plots)
            if show_plots:
                misc.repair_canvas_and_show_fig(plt.gcf())
        ...
        if writer is not None:
            ...
            writer.add_figure('test/all_tasks/class_boundaries_test', plt.gcf(),
                              close=not show_plots)
            if show_plots:
                misc.repair_canvas_and_show_fig(plt.gcf())
    ...
    for i in range(len(means)):
        ...
        if writer is not None:
            ...
            writer.add_figure('train/task_%d/train_data' % i, plt.gcf(),
                              close=not show_plots)
            if show_plots:
                misc.repair_canvas_and_show_fig(plt.gcf())
        ...
        if writer is not None:
            dhandlers[-1].plot_uncertainty_map(title='Test Data',
                     ...
                     #sample_label='Training data',
                     sketch_components=True, show=False)
            writer.add_figure('test/task_%d/test_data' % i, plt.gcf(),
                              close=not show_plots)
            if show_plots:
                misc.repair_canvas_and_show_fig(plt.gcf())
        ...
        if writer is not None:
            dhandlers[-1].plot_uncertainty_map(title='Conditional Uncertainty',
                     input_mesh=input_mesh, sketch_components=True, show=False)
            writer.add_figure('train/task_%d/cond_entropy' % i, plt.gcf(),
                              close=not show_plots)
            if show_plots:
                misc.repair_canvas_and_show_fig(plt.gcf())
        ...
        if writer is not None:
            dhandlers[-1].plot_optimal_classification(title='Class-Boundaries',
                     input_mesh=input_mesh, sketch_components=True, show=False)
            writer.add_figure('train/task_%d/class_boundaries' % i, plt.gcf(),
                              close=not show_plots)
            if show_plots:
                misc.repair_canvas_and_show_fig(plt.gcf())
...
def plot_gmm_preds(task_id, data, mnet, hnet, hhnet, device, config, shared,
                   ...
                   normal_post=None, which_cond_id=-1):
    ...
    if 'bbb' in shared.experiment_type or 'ewc' in shared.experiment_type:
        ...
        _, ret_fig = pmutils.compute_acc(task_id, data, mnet, hnet,
            ...
            normal_post=normal_post, which_cond_id=which_cond_id)
    else:
        ...
        _, ret_fig = pcutils.compute_acc(task_id, data, mnet, hnet, hhnet,
            device, config, shared, split_type="test", return_entropies=True,
            return_pred_labels=True, deterministic_sampling=True,
            in_samples=input_mesh[2], which_cond_id=which_cond_id)
    ...
    data.plot_uncertainty_map( \
        ...
        sketch_components=True, show=False)
    writer.add_figure('test/task_%d/pred_entropies' % task_id, plt.gcf(),
    ...
    data.plot_optimal_classification(title='Predicted Class-Boundaries',
        ...
        sample_inputs=other_means,
        sketch_components=True, show=False)
    writer.add_figure('test/task_%d/pred_class_boundaries' % task_id, plt.gcf(),
                      tb_step, close=not config.show_plots)
    if config.show_plots:
        misc.repair_canvas_and_show_fig(plt.gcf())
        ...
        for ii in range(10):
            if 'bbb' in shared.experiment_type:
                _, ret_fig = pmutils.compute_acc(task_id, data, mnet, hnet,
                    ...
                    in_samples=input_mesh[2], num_w_samples=1, which_cond_id=which_cond_id)
            else:
                ...
                _, ret_fig = pcutils.compute_acc(task_id, data, mnet, hnet,
                    hhnet, device, config, shared, split_type="test",
                    return_entropies=True, return_pred_labels=True,
                    deterministic_sampling=False, in_samples=input_mesh[2],
                    num_w_samples=1, which_cond_id=which_cond_id)
            ...
            data.plot_uncertainty_map( \
                ...
                sample_inputs=other_means,
                sketch_components=True, show=False)
            writer.add_figure('test/task_%d/single_samples_pred_entropies' \
            ...
            data.plot_optimal_classification(title='Predicted Class-Boundaries',
                ...
                sketch_components=True, show=False)
            writer.add_figure(\
                'test/task_%d/single_samples_pred_class_boundaries' % task_id,
                plt.gcf(), ii, close=not config.show_plots)
            if config.show_plots:
                misc.repair_canvas_and_show_fig(plt.gcf())

def plot_gmm_preds_sofa(task_id, data, mnet, hnet, hhnet, device, config, shared,
                   logger, writer, tb_step, draw_samples=False,
                   normal_post=None, which_cond_id=-1):
    """Visualize the predictive entropy over the whole input space. ALL CLASSES SO FAR.
    ...
    data.plot_optimal_classification(title='Predicted Class-Boundaries',
        input_mesh=input_mesh, mesh_modes=ret_fig.pred_labels.reshape(-1, 1),
        sample_inputs=other_means,
        sketch_components=True, show=False, modes_so_far=shared.num_trained * data.num_classes)
    writer.add_figure('test/test_all_tasks_class_boundaries_test_SOFAR', plt.gcf(),
                      tb_step, close=not config.show_plots)
    if config.show_plots:
        misc.repair_canvas_and_show_fig(plt.gcf())

def plot_gmm_preds_proj(task_id, data, mnet, hnet, hhnet, device, config, shared,
                   logger, writer, tb_step, draw_samples=False,
                   normal_post=None, which_cond_id=-1, projection=False):
    """Visualize the predictive entropy over the whole input space.
    ...
    if not projection:
        ...
        for projection_v_id in range(task_id+1):
            ...
            if 'bbb' in shared.experiment_type or 'ewc' in shared.experiment_type:
                ...
                _, ret_fig = pcutils.compute_acc(task_id, data, mnet, hnet, hhnet,
                    device, config, shared, split_type="test", return_entropies=True,
                    return_pred_labels=True, deterministic_sampling=True,
                    in_samples=input_mesh[2], which_cond_id=which_cond_id, projection=True, projection_aware=projection_v_id)
            ...
            data.plot_uncertainty_map( \
                ...
                sample_inputs=other_means,
                sketch_components=True, show=False)
            writer.add_figure('test/task_%d/proj_%d_pred_entropies' % (task_id, projection_v_id), plt.gcf(),
                            tb_step, close=not config.show_plots)
            if config.show_plots:
                misc.repair_canvas_and_show_fig(plt.gcf())
            ...
            data.plot_optimal_classification(title='Predicted Class-Boundaries',
                ...
                sample_inputs=other_means,
                sketch_components=True, show=False)
            writer.add_figure('test/task_%d/proj_%d_pred_class_boundaries' % (task_id, projection_v_id), plt.gcf(),
                            tb_step, close=not config.show_plots)
            if config.show_plots:
                misc.repair_canvas_and_show_fig(plt.gcf())
...
def plot_gmm_prior_preds(task_id, data, mnet, hnet, hhnet, device, config,
                         ...
                         prior_theta=None, which_cond_id=-1):
    ...
    if 'bbb' in shared.experiment_type:
        _, ret_fig = pmutils.compute_acc(task_id, data, mnet, hnet,
            ...
            w_samples=prior_samples, which_cond_id=which_cond_id)
    else:
        assert 'avb' in shared.experiment_type or 'ssge' in \
            shared.experiment_type
        _, ret_fig = pcutils.compute_acc(task_id, data, mnet, hnet, hhnet,
            device, config, shared, split_type=None, return_entropies=True,
            return_pred_labels=True, deterministic_sampling=True,
            in_samples=input_mesh[2], w_samples=prior_samples, which_cond_id=which_cond_id)
    ...
    data.plot_uncertainty_map( \
        ...
        sample_inputs=other_means,
        sketch_components=True, show=False)
    writer.add_figure('test/task_%d/prior_pred_entropies' % task_id, plt.gcf(),
    ...
    data.plot_optimal_classification(title='Prior Predicted Class-Boundaries',
        ...
        sample_inputs=other_means,
        sketch_components=True, show=False)
    writer.add_figure('test/task_%d/prior/pred_class_boundaries' % task_id,
    ...
    for ii in range(min(10, len(prior_samples))):
        if 'bbb' in shared.experiment_type:
            _, ret_fig = pmutils.compute_acc(task_id, data, mnet, hnet,
                ...
                deterministic_sampling=False,
                disable_lrt=config.disable_lrt_test,
                in_samples=input_mesh[2], w_samples=[prior_samples[ii]], which_cond_id=which_cond_id)
        else:
            assert 'avb' in shared.experiment_type or 'ssge' in \
                shared.experiment_type
            _, ret_fig = pcutils.compute_acc(task_id, data, mnet, hnet,
                ...
                return_entropies=True, return_pred_labels=True,
                deterministic_sampling=False, in_samples=input_mesh[2],
                w_samples=[prior_samples[ii]], which_cond_id=which_cond_id)
        ...
        data.plot_uncertainty_map( \
            ...
            sample_inputs=other_means,
            sketch_components=True, show=False)
        writer.add_figure('test/task_%d/prior_single_samples_pred_entropies' \
            % task_id, plt.gcf(), ii, close=not config.show_plots)
        if config.show_plots:
            misc.repair_canvas_and_show_fig(plt.gcf())
        ...
        data.plot_optimal_classification( \
            ...
            sketch_components=True, show=False)
        writer.add_figure(\
            'test/task_%d/prior_single_samples_pred_class_boundaries' % task_id,
            plt.gcf(), ii, close=not config.show_plots)
        if config.show_plots:
            misc.repair_canvas_and_show_fig(plt.gcf())
