...
def setup_summary_dict(config, shared, experiment, mnet, hnet=None,
    ...
    # Confusion matrices as in FACIL
    summary['confusion_matrix_acc_tag'] = np.zeros((config.num_tasks, config.num_tasks))
    summary['confusion_matrix_acc_taw'] = np.zeros((config.num_tasks, config.num_tasks))
    summary['confusion_matrix_forg_tag'] = np.zeros((config.num_tasks, config.num_tasks))
    summary['confusion_matrix_forg_taw'] = np.zeros((config.num_tasks, config.num_tasks))
...
def compute_acc(task_id, data, mnet, hnet, hhnet, device, config, shared,
                ...
                w_samples=None, which_cond_id=-1, inspect_first=0, 
                return_wnorms=False, return_sim_layers=False, logger=None, projection=False, projection_aware=-1):
    ...
    The which_cond_id is triple checking that task embedding is indicated 
    (otherwise please pass -1) - be careful, if the number of units is different!
    ...
    elif config.cl_scenario == 3 and (not config.split_head_cl3 or config.fixed_embedding != -1):
    ...
    if inspect_first > 0:
        return_vals.inspect_first = None

    if return_wnorms:
        return_vals.wnorms = None

    if return_sim_layers:
        sim_layers = np.eye(num_w_samples)
        prev_weights = []
    ...
    for j in range(num_w_samples):
        ...
        if w_samples is not None:
            ...
            if projection:
                if projection_aware == -1:
                    v = shared.projection_v[task_id]
                else:
                    v = shared.projection_v[projection_aware]
                z = z - ((z @ v) * (1/torch.norm(v)**2)).reshape(-1,1) @ v.reshape((1,-1))
            weights = hnet.forward(uncond_input=z, weights=hnet_theta)

        if return_wnorms:
            if j == 0:
                wnorms = np.empty(num_w_samples)
            
            wnorms[j] = torch.cat([p.detach().flatten() \
                    for p in weights]).cpu().norm(p=2)
            
        if return_sim_layers:
            prev_weights.append(weights)
            for prev in range(j):
                sim_layers[prev][j] = compute_weight_sim(weights, prev_weights[prev])
                sim_layers[j][prev] = sim_layers[prev][j]
            sim_layers[j][j] = compute_weight_sim(weights, weights)
        ...
        while n_processed < num_samples:
            ...
            if inspect_first > 0 and inspect_first < curr_bs and inspect_first > n_processed:
                if j == 0:
                    return_vals.inspect_first = None
                    out_sel = np.empty((num_w_samples, inspect_first, softmax_width))
                    pred_sel = np.empty((num_w_samples, inspect_first))
                    std_sel = np.empty(num_w_samples)
                    acc_sel = np.empty(num_w_samples)
                labels_sel = labels[0:inspect_first]
                X_sel = X[0:inspect_first, :]
                Y_sel = mnet.forward(X_sel, weights=weights, **kwargs)
                if allowed_outputs is not None:
                    Y_sel = Y_sel[:, allowed_outputs]
                out_sel[j, :, :] = F.softmax(Y_sel / ST, dim=1).detach().cpu().numpy()
                pred_sel[j] = np.argmax(out_sel[j], 1)
                std_sel[j] = out_sel[j, :, :].std()
                acc_sel[j] = 100. * np.sum(pred_sel[j] == labels_sel) / inspect_first
    ...
    if inspect_first > 0:
        mean_sel = out_sel.mean(axis=0)
        pred_all_sel = np.argmax(mean_sel, axis=1)
        all_acc = 100. * np.sum(pred_all_sel == labels_sel) / inspect_first

        return_vals.inspect_first_pred_j = pred_sel
        return_vals.inspect_first_pred_all = pred_all_sel
        return_vals.inspect_first_acc_j = acc_sel
        return_vals.inspect_first_acc_all = all_acc
        return_vals.inspect_first_std = std_sel
    ...
    if return_pred_labels:
        ...
        if labels is not None:
            prediction_confusion = np.zeros((softmax_width, softmax_width))
            for i_class in range(softmax_width):
                relevant_inds = labels == i_class
                num_relevant_inds = np.count_nonzero(relevant_inds)
                if num_relevant_inds != 0:
                    for j_class in range(softmax_width):
                        prediction_confusion[i_class][j_class] = \
                            np.count_nonzero(pred_labels[relevant_inds] == j_class) / num_relevant_inds
            return_vals.prediction_confusion = prediction_confusion

    if return_entropies:
        # We use the "maximum" trick to improve numerical stability.
        entropies_by_class = pred_dists * np.log(np.maximum(pred_dists, 1e-5))
        entropies = - np.sum(entropies_by_class ,axis=1)
        # return_vals.entropies = - np.sum(pred_dists * np.log(pred_dists),
        #                                 axis=1)
        assert entropies.size == X.shape[0]

        # Normalize by maximum entropy.
        max_ent = - np.log(1.0 / softmax_width) # len(allowed_outputs)
        return_vals.entropies = entropies / max_ent

        if labels is not None:
            entropies_confusion = np.zeros((softmax_width, softmax_width))
            for i_class in range(softmax_width):
                relevant_inds = labels == i_class
                entropies_confusion[i_class] = entropies_by_class[relevant_inds].mean(axis=0)
            return_vals.entropies_confusion = entropies_confusion
    ...
    if return_wnorms:
        return_vals.return_wnorms = wnorms

    if return_sim_layers:
        return_vals.sim_layers = sim_layers
...
def compute_weight_sim(model1, model2):
    vector1 = model1[-2].detach().cpu().numpy().flatten()
    vector2 = model2[-2].detach().cpu().numpy().flatten()
