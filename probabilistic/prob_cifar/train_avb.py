...
from probabilistic import train_vi_proj as tvi_proj
...
def run(config, experiment='split_mnist_avb'):
    ...
    ### Make hhnet frozen if fixed embedding
    detach_grad_emb = False
    if config.task_id_emb != -1: # TODO
        test_task_id_emb = config.task_id_emb
    else:
        test_task_id_emb = -1
    if config.fixed_embedding != -1:
        test_task_id_emb = config.fixed_embedding
        assert test_task_id_emb == 0
    ...
    vs = torch.normal(torch.zeros(config.num_tasks, shared.noise_dim), config.latent_std).to(device)
    vs = vs.detach()
    shared.projection_v = vs
    ...
    for i in range(config.num_tasks):
        ...
        # Loading the net if applicable
        if config.load_from_base != "" and i == 0:
            #if os.path.isdir() #TODO: support for more tasks
            task_id_emb = i
            logger.info(f"Loaded from {config.load_from_base}")
            pmutils.load_networks_from_base(config.load_from_base, i, device, logger, mnet, hnet, hhnet, dis=None)
            shared.theta_prev = None
            if hhnet is not None:
                shared.theta_prev = hhnet.forward(cond_id=task_id_emb)
            if config.fixed_embedding != -1:
                last_task_id_emb = i #FIXME
            else:
                last_task_id_emb = i
            shared.prev_task_id.append(int(last_task_id_emb))
        
        # Train the network.
        else:
            if config.fixed_embedding != -1:
                task_id_emb = config.fixed_embedding
            else:
                task_id_emb = i

            if config.distill_iter == -1:
                tvi_proj.train(i, data, mnet, hnet, hhnet, dis, device, config, shared,
                    logger, writer, method=method, task_id_emb=task_id_emb)
            else:
                assert hhnet is not None
                # Train main network only.
                tvi_proj.train(i, data, mnet, hnet, None, dis, device, config, shared,
                        logger, writer, method=method, task_id_emb=task_id_emb)
        ...
        if config.full_test_interval != -1:
            if config.full_test_interval == 1 or \
                    i == config.num_tasks-1 or \
                    (i > 0 and i % config.full_test_interval == 0):
                test_ids = None # Test on all tasks.
            else:
                test_ids = [i] # Only test on current task.
        tvi_proj.test(dhandlers[:(i+1)], mnet, hnet, hhnet, device, config, shared,
             logger, writer, test_ids=test_ids, method=method, fixed_embedding=test_task_id_emb, optimal_embedding=test_task_id_emb) #TODO
        ...
        if config.save_after_base and i == 0:
            pmutils.checkpoint_nets_after_base(config, shared, i, mnet, hnet, hhnet=hhnet, dis=None)
            #TODO: checkpoint_bn_stats
